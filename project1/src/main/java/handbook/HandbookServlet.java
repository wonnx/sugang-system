...
public class HandbookServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println("handbook-servlet-doget: database connection success");
        ...
        String sql =
                "SELECT\n" +
                "\tT8.class_id, 수업번호, 학수번호, 교과목명, T8.credit, 교강사이름, 수업시간,\n" +
                "    CONCAT(수강 + IFNULL(신규, 0), ' / ', 정원) AS 수강정원, 강의실\n" +
                "FROM (\n" +
                "\tSELECT \n" +
                "\t\tT7.class_id, 수업번호, 학수번호, 교과목명, T7.credit, 교강사이름,\n" +
                "\t\tgroup_concat(수업시간 SEPARATOR ', ') AS 수업시간, 수강, 정원, 강의실\n" +
                "\tFROM (\n" +
                "\t\tSELECT \n" +
                "\t\t\tT6.class_id,\n" +
                "\t\t\tT6.class_no AS 수업번호,\n" +
                ...
                "    )AS T7\n" +
                "\tGROUP BY class_id, 수업번호, 학수번호, 교과목명, credit, 교강사이름, 수강, 정원, 강의실\n" +
                ") AS T8 LEFT JOIN (\n" +
                "\tSELECT class_no, count(class_no) AS 신규\n" +
                "\tFROM sugang\n" +
                "\tGROUP BY class_no\n" +
                ") AS T9\n" +
                "ON T8.수업번호 = T9.class_no;";
        try{
            ...
            if(page.equals("handbook")){
                response.sendRedirect("../component/pages/handbook/handbook.jsp");
            }else if(page.equals("enrolment")){
                response.sendRedirect("../component/pages/enrolment/enrolment.jsp");
            }
